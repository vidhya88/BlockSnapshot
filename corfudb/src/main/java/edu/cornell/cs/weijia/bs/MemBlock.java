...
public abstract class MemBlock implements Snapshottable {
  ...
  public enum BlockImplement{
	  ...
	  public  void write(int offset, int length, byte[]buf)throws BlockSnapshotException{
		  wl.lock();
			try{
				if(!restore_mode)
					shared_log.writeToLog(String.valueOf(Command.WRITE.getNumVal()),offset,length,buf,new String(this.blockid));
   ...
   * @return
   */
  static public MemBlock createMemBlock(BlockImplement bi,String pblockID){
    ...
    if(pblockID==null)
    	mb.blockid=UUID.randomUUID().toString();
    else
    	mb.blockid=pblockID;
    if(!restore_mode)
    	shared_log.writeToLog(String.valueOf(Command.CREATE_MEM_BLOCK.getNumVal()), bi.toString(),mb.blockid);
	...
	//acquire write lock
	 // write to shared log
	  ...
	  try{
		  ...
		  //write to CORFU Log
		  if(!restore_mode)
			  shared_log.writeToLog(String.valueOf(Command.CREATE_SS.getNumVal()),sid,this.blockid);
		  createSS(sid);
	  }
		catch(Exception exp)
		{
  ...
  public void deleteSnapshot(Object sid)
  {
	  wl.lock();
		try{
			//write to shared log
			if(!restore_mode)
				shared_log.writeToLog(String.valueOf(Command.DELETE_SS.getNumVal()),sid,this.blockid);
			deleteSS(sid,false);
		}
